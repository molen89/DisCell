...
```
input = [ log1p( x_i / ℓ_i · median(ℓ) ) ,  log ℓ_i ,  onehot(t_i) ]
```

Library-normalise before `log1p`. Raw `log1p(x)` entangles depth with composition — a 300-count
and a 50-count cell of the same type look very different — and since `ℓ_i` is conditioned on
rather than modelled, `z` should not spend capacity on it. Total counts are still weakly
state-informative, so `log ℓ_i` goes in as one extra scalar rather than being discarded.
...
### 4.5 Frozen neighbours, and the batched forward pass
...
**`ρ̄_i` reaches two hops** — it needs `ρ_j`, hence `w_j`, hence `c_j`, hence `N(j)`. Batch by
**contiguous spatial tile** and this is cheap, because the halo is shared rather than disjoint:
...
query. A cell 35 µm away across a pruned edge is not in ring 1 even though a radius search returns
it, and if the halo and the leak kernel disagree, `ρ̄` is silently wrong.
...
Both roles read the same node ordering from **two feature tensors**, so a cell that is a seed and
also another seed's neighbour just reads different rows. **Filter edges to `dst ∈ seeds ∪ ring1`**:
...
dropping `w_j` strips the local spatial programme out of `ρ̄`, and whatever the leak term fails to
absorb is explained by `w_i` instead — **inflating apparent spatial effects**, the exact failure the
`κ` sweep exists to guard against.
...
```
Pen = Σ_t p(t) · ½[ log det Σ̂_{v|t} + log det Σ̂_{z|t} − log det Σ̂_{[z,v]|t} ],   v = [y, PCs(Φ)]
```

Three estimation details, none optional at realistic batch sizes. Keep `v` small — 8–16 PCs of `Φ`
puts the joint block at 20–30 dims, far better conditioned, and the top PCs carry the niche signal.
Accumulate the covariances as **EMAs across batches**, sized for an effective sample of ~10×dim per
type, since a spatial tile contains few cells of any rare type. And shrink toward
**`diag(Σ̂)`, not `+λI`**: `Σ̂ ← (1−a)Σ̂ + a·diag(Σ̂)` damps only the correlations, whereas `+λI`
inflates every variance by an amount that depends on how ill-conditioned the estimate was — biasing
`log det` upward most for rare types, i.e. weakening the penalty exactly where data is thinnest.
Report the fraction of cells below the support floor; those are unregularised.
