...
One consequence of the softmax over a Delaunay neighbourhood should be stated: the aggregated context is a reweighted composition and is exactly invariant to the number of neighbours, so one neighbour of a given type gives the same context as several. On this tessellation the degree is nearly constant, so a fraction determines a count and the blindness costs nothing; an attention sink, a fixed null logit that restores a saturating dependence on the count, is implemented and left off for that reason, and would only become meaningful on a graph whose degree varies, where the invariance target would have to grow with it.
