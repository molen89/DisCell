...
| 8.15 | α_w multiplier ladder {1,2,5,10,25}×1/ℓ̄ + 0.1 reference, 4 datasets × 3 seeds, final config, guards + held-out deviation gain; rule pre-registered (R19) | α_w 0.1 is 14–143× bound-equivalent; w ≈ m_ψ by construction | smallest passing m with held-out gain, else keep 0.1 and reframe | **running 2026-09-24** |
| 8.16 | Per-block, scale-free invariance probe (composition vs image, V-information gain) + MLP grader; re-grade every decision input and the baselines at the accepted checkpoint (R20, R22) | the pooled MSE probe is >99 % image; composition ungraded | new guard applied to all tables; ladder read only with it | **running 2026-09-24** |
