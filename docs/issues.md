...
- **W-qp1 (2026-09-23).** `sweep.py` and `report.py` rebuild `DisCell` field by field and omit `class_mean_prior`, `query`, `prior_type_free`; ablation runs (6b.5 arm iii, 8.10) cannot be reloaded by them. `validate.load_run` is complete. Fix: route both through `load_run` or add the `setdefault` pass-throughs. Open.

- **W-q1 (2026-09-23).** Queue scripts with DONE markers must key the marker by dataset as well as step and run name; the collapse-fix queue did not and would have skipped 12 FF fits. Fixed in `scripts/queue_2026-09-23_wcollapse.sh`; check `queue_2026-09-23_queryprior.sh` (single dataset, unaffected) and any future multi-dataset queue.

- **W-qp1 addendum (2026-09-24).** `report.py`/`sweep.py` also omit `kappa_mode`; a gene-mode checkpoint fails loudly there. Same fix as W-qp1.
