...
**8.17 confirmation, GSE interim (06:40).** comp3 on GSE (2 seeds vs the fresh reference pair): MLP composition fraction 0.71 → **0.53**, ridge 0.41 → **0.25**; recon −7.2027 → −7.2103 (−0.006 / −0.010 paired), NMI 0.633 → 0.627. By the pre-stated rule comp3 is *not confirmed*: recon and NMI fall outside the control envelope — an envelope built from two seeds with sd 0.0017 (recon) and a miss of 0.0002 (NMI). Same pattern as every two-seed envelope this week: the rule's tolerance is the control's luck. Substantively: composition leakage down 25–40 % on GSE as on ovarian, at a recon cost of 0.006–0.010 nats/count and ~0.006 NMI. Ovarian third seed, comp5 dose and FF land by ~08:45; the adoption call then goes to the author on the trade (invariance is the headline claim; the cost is small and would be stated).

### Lineage-level labels (R9, motivation, 2026-09-25 07:20)

**Why.** The model conditions everything on t. Where t encodes a state (proliferative, VEGFA⁺, inflammatory, activated) or a location (tumour-associated, stroma-associated, lining a cyst), that part of the response is handed to the label, where neither w nor the invariance can see it, and some type-level findings become partly circular. The manuscript already says labels are lineage-level; the fits do not follow it. **Rule:** merge states and locations into their lineage; keep developmentally distinct subtypes (AT1/AT2; capillary/venous/lymphatic endothelium; ciliated/secretory/basal; alveolar vs adventitial fibroblasts; alveolar vs interstitial macrophages; airway vs vascular smooth muscle). **Grey case decided by data, not name:** SOX2-OT⁺ tumour cells (ovarian, 9.7 %) stay a lineage iff their expression differs from other tumour cells beyond SOX2-OT itself by a clone-sized margin (≥ 50 DE genes at |log2FC| > 1, BH q ≤ 0.01, on the tumour-only comparison) AND they form contiguous patches (spatial autocorrelation of the label within tumour cells well above a within-tumour permutation); otherwise they merge. **Risk named:** over-merging a spatially clustered true sub-lineage turns its expression into location-correlated within-type variation, which w absorbs as a "response" — the mirror image of the circularity being fixed.

**Plan.** (1) Draft mapping tables, read-only: ovarian 18 → ~11; GSE curated 35 → ~25 with the 0.2 % "Proliferating" class reassigned per cell to a lineage by markers with cycle genes excluded, the same mapping applied to the dual section; lung FFPE (32 graphclust clusters) and FF (38) mapped to lineages from a marker-score table per cluster, mixed clusters flagged. (2) Author reviews; tables committed before any refit. (3) New obs column `lineage` in each bundle; `--label-key lineage`; then the single re-pin (reference triples, uncontrolled references, κ sweep at the reference budget, GSE held-out), resolVI refit on all five bundles (it trains on the label), SIMVI/MintFlow battery columns recomputed. **Consequences stated in advance:** NMI values are not comparable to today's (fewer classes); the 0.9 × running-max guard survives, the 0.63 reference number does not; cycling and VEGFA⁺ become states the model must place; invariance gets harder. What is wished for: the response and invariance findings survive with more within-type variation to explain — "the numbers do not become horrible" (author).

### Metrics package for the final tables (motivation, 2026-09-25 07:30)

Four additions the reviews name, all evaluation-side, built now and applied to the final runs once they exist: (1) **tile-bootstrap intervals** as the standard wrapper on every headline read (transport gap closed and fraction of ceiling, probe fractions, cycle R², NMI, I(niche;w) excess), 200 µm tiles, 1000 draws, with one stated multiplicity treatment across the κ grid (Holm over the grid's pairwise comparisons) (R33); (2) **held-out reconstruction as a model-comparison score** (R24): beside the autoencoding score (own posterior), a context-only decode (z at its type mean, w at the prior mean — no use of the cell's own counts) and an intrinsic-only decode (posterior z, prior w), all per count, with the type-profile reference; the paper states which one drove checkpoint selection; (3) **the projection test** the implementation table names (S53): full 384-d Φ in c vs a learned linear projection to 32, ovarian, 3 seeds, 200/20 at the final default, compared on held-out reconstruction and the guards — run now at current labels for the sentence, re-read at final labels if the default changes; (4) **a pure-training timing mode**: N epochs with no evaluation, with and without Φ, per dataset, reporting s/epoch, s/evaluation and peak GPU memory, beside the baselines' fit times. Nothing here changes a model default.

**Hand-off 2026-09-25 ~07:45 (author closing briefly).** Detached: adversary confirmation queue (→ `DECISION_ADVC.json`, `experiments/adv_confirm.md`, ~08:45). In progress inside the session, may be cut and relaunched: lineage mapping tables (partial CSVs under `data/datasets/<ds>/labels/`, report `scripts/logs/lineage_2026-09-25/`), metrics package (`scripts/logs/metrics_2026-09-25/`). On return: read `adv_confirm.md` → settle the composition weight; review the mapping tables → commit them; then one queue: relabel, re-pin at the settled default (reference triples 500/40, uncontrolled refs, κ sweep at reference budget, GSE held-out), resolVI refits, battery columns, metrics package on the finals, archive of superseded runs.

### Lineage mapping tables drafted (R9, results, 2026-09-25 ~08:00; nothing applied)

*Tables `data/datasets/<ds>/labels/lineage_map_proposed.csv` (five bundles), GSE per-cell reassignment files, lung/FF marker-score tables; report `scripts/logs/lineage_2026-09-25/AGENT_REPORT.md`.*

**Headline: the ovarian "SOX2-OT⁺ Tumor Cells" class (9.7 %) is not a tumour clone; it is mostly low-depth non-tumour cells.** It passes the pre-registered keep rule (2,085 DE genes, 78 % same-label tumour neighbours vs 19 % shuffled) — but median depth is 27 transcripts vs 333 for tumour cells, 75 % of the cells have zero SOX2-OT counts, EPCAM and PAX8 are 10× lower, an NNLS decomposition of the pooled profile reads Macrophages 0.52 / T-NK 0.09 / tumour 0.08, depth-matched DE shows macrophage and fibroblast genes up and tumour genes down, and only 14 % of cells sit nearest a tumour centroid. **Recommendation: map to Unassigned, not to Tumour** (the author decides; the table follows the rule and keeps it). Lesson for the rule: at these cell counts every merged tumour state also passes both thresholds (Proliferative 220 DE / 0.50 vs 0.24; VEGFA⁺ 277 / 0.51 vs 0.05; Inflammatory 77 / 0.70 vs 0.01; Lining Cyst 782 / 0.61 vs 0.01), so DE-plus-contiguity does not separate a clone from a state; only the depth/marker/decomposition read did.

**Second grey case: "Malignant Cells Lining Cyst" is mesothelial** (CALB2, PRG4, BNC1, PDPN, UPK3B on; EPCAM, PAX8, ESR1 off) — proposed as its own lineage "Mesothelial-like cyst lining", not merged into Tumour.

**Counts:** ovarian 18 → 13 (12 with SOX2-OT⁺ → Unassigned); GSE 35 → 30, one mapping for both sections, Proliferating / EC-activated / Inflammatory / Activated / Myofibroblasts reassigned per cell (expected accuracy ~0.75; margins reported); lung FFPE 32 clusters → 20 classes on the GSE lung vocabulary; FF 38 clusters → 10 classes (Tumour, Macrophages, Ovarian stroma, Fibroblasts, T/NK, Unassigned, Plasma, Endothelial, Pericytes, Smooth muscle). Mixed clusters flagged: lung 3, 10, 24, 30 (+5, 19, 21, 28 by hand); FF 14, 20, 18, 22 (immune next to tumour carrying tumour signal — the leakage signature), 28, 30, 31, 37, and 9 (low depth → Unassigned). Tumour-/stroma-associated fibroblasts → one class (784 DE genes = CAF programme, a state); the two endothelial classes → one (tip state vs weakly venous; grey). GSE Myofibroblasts reassigned 51/49 to alveolar/adventitial (grey; 76–79 % from one core). Panel gaps: FF and GSE lack KRT8, SFTPC, VWF, DCN, LUM, COL1A1, MYH11, ACTA2, PTPRC, NKG7 — substitutes listed.

**For the author's review:** (1) SOX2-OT⁺ → Unassigned (recommended) or Tumour; (2) cyst lining as mesothelial lineage (recommended); (3) the grey merges above; (4) FF's mixed immune-near-tumour clusters — keep the lineage call (the tumour signal is leakage, which is what the model is for) with the flag recorded. Tables are committed before any refit.

**Metrics package launched (07:13, detached; read-out ~12:30).** Built: `experiments/bootstrap.py` (tile bootstrap over held-out cells for every headline read, Holm over the κ grid; planted coverage 0.93 with tiles vs 0.57 with a cell-level bootstrap — the tile unit matters), `experiments/recon_modes.py` (full / intrinsic-only / context-only / type-profile decodes with CIs), `--phi-proj D`, `--time-only N` (+ arms Φ zeroed and Φ dropped), `envelope_tables.py --ci`, timing and projection tables. **First read on GSE `final_s0` (own / dual section): full posterior −7.2047 / −7.2193; intrinsic-only (posterior z, prior w) −7.2048 / −7.2194; context-only (type-mean z, prior w) −7.3733 / −7.3914; type profile −7.3739 / −7.3899.** Two facts for the paper: replacing the posterior w by its prior changes held-out reconstruction by 1e-4 nats/count (the per-cell w channel is closed at α_w 0.1, as R19 says, now in likelihood terms); and the context-only decode sits on the type profile, so the response channel's contribution to reconstruction is negligible in likelihood — its value is in predicting held-out shifts (transport), not in fitting counts. That is the honest framing of what w does. `validate.load_run` gained the `phi_proj` pass-through. The pre-existing R26 test failure (NMI re-eval noise) is confirmed on a clean tree (issue T-r26).

### Adversary composition weight, confirmation (8.17, interim results, 2026-09-25 08:00; FF pending)

*GSE 2 seeds, ovarian 3 seeds (control = the five α_w 0.1 fits pooled); FF comp3 running. `experiments/adv_confirm.md`.*

| read | GSE control → comp3 → comp5 | ovarian control → comp3 → comp5 |
|---|---|---|
| ridge composition, fraction of uncontrolled | 0.41 → **0.25** → 0.19 | 0.41 → **0.25** → 0.22 |
| MLP composition excess (nats/col) | 0.020 → 0.015 → 0.013 | 0.029 → 0.023 → 0.018 |
| mirror R² | 0.028 → 0.025 → 0.025 | 0.038 → 0.032 → 0.032 |
| I(niche;w) excess | 0.50 → 0.56 → 0.63 | 0.64 → 0.68 → 0.66 |
| Read A own gap closed | 0.57 → **0.65** → 0.75 | 0.72 → **0.78** → 0.80 |
| NMI | 0.633 → 0.627 → 0.619 | 0.650 → 0.655 → 0.636 |
| cycle_z | 0.476 → 0.486 → 0.489 | 0.525 → 0.517 → 0.480 [0.42, 0.53] |
| recon | −7.2027 → −7.2103 → −7.2128 | −7.1818 → −7.1765 → −7.1726 |

**Reading.** Weighting composition ×3 in the adversary halves the linear composition leakage on both slides, cuts the nonlinear leakage by a quarter, lowers the mirror, and — the part not anticipated — **raises the response channel's niche information and the transport gap closed** (GSE 0.57 → 0.65, ovarian 0.72 → 0.78): what the encoder is forced to drop from z reappears in w where it belongs. NMI and cycle_z are unchanged at ×3; recon −0.008 nats on GSE, +0.005 on ovarian. **×5 goes further on leakage and transport (GSE gap 0.75) but starts to cost: NMI −0.014 on both, ovarian cycle_z spread 0.42–0.53.** By the pre-stated rule comp3 fails only the GSE recon/NMI envelope clause (two-seed control, sd 0.002 / miss 0.0002) — the same two-seed-envelope artefact as before. **Recommendation (author's call after FF lands ~09:00): adopt composition weight 3 as default**; the trade is −0.008 nats/count on GSE for halved composition leakage and +0.06–0.08 gap closed, and the paper states it. Then the single re-pin at lineage labels.

**8.17 confirmation complete (08:33): FF.** comp3 vs control (2 seeds): ridge composition fraction 0.18 → **0.12**, MLP excess 0.014 → 0.010, mirror 0.053 → 0.048, I(niche;w) 0.59 → **0.66**, Read A own gap closed 0.75 → **0.84**; costs: NMI 0.624 → 0.602 (−0.022), cycle_z 0.782 → 0.757 (−0.025), recon −0.005. **Rule: not confirmed** (FF NMI and cycle_z outside the envelope; GSE recon/NMI by a hair). **Pattern on three slides:** linear composition leakage halved everywhere, nonlinear −25–30 %, mirror down, w's niche information up, transport gap closed up 0.06–0.09; guard costs are dataset-specific — none on ovarian, −0.008 nats on GSE, −0.02 NMI and −0.025 cycle_z on FF (where cycle_z remains 0.76, far above every other slide). **Recommendation stands: adopt composition weight 3 as default and state the FF cost.** The two headline claims (invariance, transport) improve on every slide; the costs land on secondary reads within one slide. Author's call at launch (`ADV_COMP_WEIGHT`).

**Final queue built, not launched (`scripts/queue_2026-09-25_final_lineage.sh`; `apply_lineage.py`; `archive_2026-09-25.py --dry-run`).** Dry-run class counts at the recommended choices: ovarian 18 → 12 (Unassigned 10.3 % with SOX2-OT⁺ folded in), GSE 35 → 30 (both sections, one vocabulary; Myofibroblasts = 26 % of the training core, split per cell), lung 32 → 20, FF 38 → 10. **Cost estimate: ~74 GPU-h (~37 h wall), FF ~47 of it; the κ sweep at the reference budget with full per-run reads is 49 GPU-h of the total.** Blockers before launch: (1) name matchers in `transport.tumour_band_labels` and `validate.landmark_inventory` look for "Tumor Cells" / "Malignant" / "Smooth Muscle" and would miss the new spellings — a case-insensitive fix in evaluation code is ordered (tumour band = classes matching "tumou?r" only; mesothelial cyst lining and Unassigned are not tumour); (2) the sweep's κ = 0.1 point duplicates the references — symlinked instead (−8 GPU-h); (3) sweep reads reduced to battery + probe + transport mean read + κ-survival on all points, full reads (distribution transport, atlas compare, external criteria) on the reference triples only — a `SWEEP_READS=light` switch, saving ~20 GPU-h. Archive inventory: 16 GB proposed, moved only after the re-pin; `dw*`, `reference*`, `best_pre_az0.5` flagged as lacking a read-out table (kept). 14.5 GB of a failed MintFlow FF attempt in the baselines results can be deleted.
**Label matchers (09:10).** `discell/model/labels.py`: `is_tumour` = `tumou?r|malignant|carcinoma` and not `mesothelial|unassigned|lining`; `is_smooth_muscle`, `is_endothelial`; used in `transport.tumour_band_labels` and `validate.landmark_inventory` (a local-variable shadowing bug that would have raised was fixed on the way). 22 + 69 tests. **Semantic correction, recorded:** the old "Malignant Cells Lining Cyst" class no longer counts as tumour for the tumour-band and interface reads even under the old vocabulary — its markers are mesothelial — so tumour-band numbers on old-label runs are not bit-reproducible from here; the old artefacts on disk stand as computed, and the final runs use the new definition.
**Matcher correction (09:30).** `is_tumour` also excludes `associated|fibroblast|endothel|macrophage|stroma|immune|t cell`, so "Tumor Associated Fibroblasts / Endothelial Cells" are not tumour under the old vocabulary either (the builder's preflight caught it). Old-label tumour-band reads now differ from the published ones only by the cyst-lining exclusion. 23 label tests + transport tests pass.

**Final queue ready (09:35), not launched.** `scripts/queue_2026-09-25_final_lineage.sh` with `SOX2OT`, `CYST`, `ADV_COMP_WEIGHT` (default 3), `SWEEP_READS` (default light: battery + probe + transport mean read + κ-survival + GSE cross-slide on every κ point; full reads on the reference triples), κ = 0.1 sweep points symlinked to the references, uncontrolled references with guard/probe/recon_modes/mean transport only. Preflight passes on 34/34 arm configs and on the label vocabulary after the matcher fix. Estimate ~50 GPU-h, FF ~35, about a day of wall time on two cards. Launch command in `scripts/logs/final_prep_2026-09-25/AGENT_REPORT.md`. Awaiting the author's four decisions (composition weight, lineage choices, sweep reads, housekeeping).
